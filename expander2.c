...
char *ft_expander_home(char *token, t_list **env)
{
	...
	if (!token)
		return (NULL);
	if ((token[0] == '~' && token[1] == '/') || (token[0] == '~' && ft_strlen(token) == 1))
	{
	//	param[1]++;
		var_env = ft_get_var_env(env, "..HOME");
		if (!var_env)
			return ((char *)ft_free_char_n(token, NULL, NULL, "Mem error getting \"HOME\""));
...
char *ft_add_normal(char *token, char *aux_str, int *param)
{
	...
	while (token[param[2]] && (token[param[2]] != '"' && token[param[2]] != '\''))
	{
		ptr = aux_str;
		aux_str = ft_strjoin(aux_str, letter);
		if (!aux_str)
...
char *ft_add_expander(char *token, char *aux_str, t_list **env, int *param)
{
	...
	while (token[param[2]])
	{
		crtl = token[param[2]];
		if (crtl != '"' && crtl != '\'')
			aux_str = ft_add_normal(token, aux_str, param); // todo mdifica me salgo con comillas
		else
			aux_str = ft_add_quote(token, aux_str, param); // todo modifica
		if ((aux_str) && (crtl != '\''))
			aux_str = ft_expander_dollar(aux_str, env); // añadir param para controlar avance
	}
...
{
	int param[3]; // 0 nº token 1. posisión en el token: 2 recorro en tokenen add_expander
	char *aux_str;
	char *ptr;
